...
# =========================
    ...
    **¿Qué muestran los gráficos?**  
    ...
    # =========================
    # Marimekko por comorbilidad • Predicción RF (toda la muestra)
    # =========================
    from matplotlib.patches import Patch

    COMORB_COL   = "comor"  # <-- AJUSTA al nombre de tu columna de comorbilidad
    grupo_order  = ["Sin comorbilidades", "Solo diabetes", "Solo hipertensión", "Diabetes + hipertensión"]
    riesgo_order = ["Riesgo nulo","Riesgo leve","Riesgo moderado","Riesgo severo"]

    # Colores (coherentes con tus figuras)
    colors = {"Riesgo nulo":"#1f77b4", "Riesgo leve":"#2ca02c",
              "Riesgo moderado":"#f1c40f", "Riesgo severo":"#d62728"}

    if COMORB_COL not in df_pred_all.columns:
        st.warning(f"No encuentro la columna '{COMORB_COL}'. Cámbiala en COMORB_COL.")
    else:
        # Filtra a predicciones válidas (excluye "Sin datos")
        df_mk = df_pred_all[df_pred_all["nivel_riesgo_pred"].isin(riesgo_order)].dropna(subset=[COMORB_COL]).copy()
        if df_mk.empty:
            st.info("No hay predicciones válidas para construir el Marimekko.")
        else:
            total = len(df_mk)
            # Anchuras ∝ tamaño del grupo; alturas = composición interna
            widths, Ns, comp_list = [], [], []
            for g in grupo_order:
                sub = df_mk.loc[df_mk[COMORB_COL] == g]
                Ns.append(len(sub))
                widths.append(len(sub)/total if total else 0)
                comp_list.append(sub["nivel_riesgo_pred"].value_counts().reindex(riesgo_order, fill_value=0))

            fig, ax = plt.subplots(figsize=(11.5, 6))
            x0 = 0.0
            for g, width, n_g, counts in zip(grupo_order, widths, Ns, comp_list):
                if width == 0:
                    continue
                bottom = 0.0
                for r in riesgo_order:
                    h = (counts[r]/n_g) if n_g else 0
                    ax.bar(x0 + width/2, h, width=width, bottom=bottom,
                           color=colors[r], edgecolor="white", linewidth=0.8)
                    # Etiqueta interna si el segmento es “visible”
                    if h >= 0.08:
                        ax.text(x0 + width/2, bottom + h/2, f"{int(round(h*100))}%",
                                ha="center", va="center", fontsize=10)
                    bottom += h
                # Etiqueta superior: n y porcentaje del ancho
                ax.text(x0 + width/2, 1.02, f"n={n_g}\n{width*100:.1f}%",
                        ha="center", va="bottom", fontsize=10, fontweight="bold")
                x0 += width

            # Ejes/títulos/leyenda
            ax.set_ylim(0, 1.15)
            ax.set_ylabel("Composición dentro del grupo (100%)")
            # Ticks centrados por barra
            centers = np.cumsum([0]+widths)[:-1] + np.array(widths)/2
            ax.set_xticks(centers)
            ax.set_xticklabels(grupo_order, rotation=15, ha="right")
            ax.set_title("Marimekko por comorbilidad • Predicción RF (toda la muestra)")
            ax.grid(axis="y", linestyle="--", alpha=0.25)
            legend_handles = [Patch(color=colors[r], label=r.replace("Riesgo ", "")) for r in riesgo_order]
            ax.legend(handles=legend_handles, title="Nivel de riesgo", ncol=4, bbox_to_anchor=(0.5, 1.18),
